...
user-invocable: true
description: >-
  Run an opinionated software engineering review panel over code, diffs, or design choices.
allowed-tools:
  - Bash(git diff *)
  - Bash(git log *)
  - Bash(git status *)
...
> **Preflight.** This skill dispatches eight parallel `gurus:sonnet-max` agents. This plugin ships that agent. If the dispatch fails with "unknown subagent_type: gurus:sonnet-max", run `claude plugins update gurus@laicluse-agent-fieldkit` and try again.
