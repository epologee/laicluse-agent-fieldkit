...
- `plugin-mutation`: routes machine-wide Claude and Codex plugin installs, updates, removals, enablement changes, and marketplace mutations to the operator. Claude gets the host's own permission prompt through PreToolUse `permissionDecision: "ask"`, which shows the exact pending command; every other host gets a hard deny carrying the same reason. A local Codex marketplace that already points at the primary checkout needs only candidate integration. The Claude path expects a host that understands the `ask` decision, and only asks in a permission mode where the operator actually sees the prompt: `auto`, `bypassPermissions` and an unknown mode get the deny instead. Claude, Codex.
