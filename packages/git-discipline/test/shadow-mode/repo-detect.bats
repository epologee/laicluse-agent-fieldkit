...
@test "agent-fieldkit repository URL: violation still writes to shadow log" {
  export GIT_SHIM_ORIGIN_URL="git@github.com:laicluse/agent-fieldkit.git"
...
@test "agent-fieldkit HTTPS URL: violation writes to shadow log" {
...
@test "other-repo URL: violation also writes to shadow log (universal)" {
  ...
  # Shadow log grows even for repositories other than agent-fieldkit.
  local after
  after=$(shadow_log_line_count)
  [ "$after" -eq $((before + 1)) ]
}
