...
- **Branch review works without a remote.** Second-opinion snippets now fall back from `origin/HEAD` to an existing local `init.defaultBranch` before requiring an explicit range.
