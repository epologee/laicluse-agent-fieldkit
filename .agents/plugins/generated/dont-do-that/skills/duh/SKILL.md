...
4. **Stop at the first real gate.** If the proposed action hits an inviolable gate from the session's durable instructions (for example `AGENTS.md`, `CLAUDE.md`, or the active harness policy) such as push, merge to default, deploy, destructive git, remote creation (`gh repo create`, `gh repo fork`, `git remote add`, `git remote set-url`), or any other external irreversible operation, stop there and ask. Reversible local actions (running a script, editing a file, restarting a local daemon, querying a DB, creating a local-only git repo) are not gates and do not require a check-in.
