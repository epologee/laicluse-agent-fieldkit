...
guard_no_remote_create() {
  ...
  if grep -Eq '(^|&&|;|\|\||[[:space:]])[[:space:]]*gh[[:space:]]+repo[[:space:]]+(create|fork)([[:space:]]|$)' <<< "$cmd"; then
    dd_emit_deny no-remote-create "remote creation blocked: 'gh repo create' / 'gh repo fork' spins up an account-bound repository on the forge; that is an operator decision. Ask the operator to run it through the host's manual escape/approval path, or to create the repo in the browser and tell you the URL."
  fi

  if grep -Eq '(^|&&|;|\|\||[[:space:]])[[:space:]]*git[[:space:]]+remote[[:space:]]+(add|set-url)([[:space:]]|$)' <<< "$cmd"; then
    dd_emit_deny no-remote-create "remote attach blocked: 'git remote add' / 'git remote set-url' rewires the local repo to a remote the operator may not have authorized. Ask the operator to run it through the host's manual escape/approval path."
  fi
}
