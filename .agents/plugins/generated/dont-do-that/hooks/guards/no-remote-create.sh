...
guard_no_remote_create() {
  ...
  if grep -Eq '(^|&&|;|\|\||[[:space:]])[[:space:]]*gh[[:space:]]+repo[[:space:]]+(create|fork)([[:space:]]|$)' <<< "$cmd"; then
    dd_emit_deny no-remote-create "remote creation blocked: 'gh repo create' / 'gh repo fork' creates account-bound forge state. Deleting it later is not true reversibility once the name, visibility, audit events, or notifications may have existed on the internet. Ask the operator to run it through the host's manual escape/approval path, or to create the repo in the browser and tell you the URL."
  fi

  if grep -Eq '(^|&&|;|\|\||[[:space:]])[[:space:]]*git[[:space:]]+remote[[:space:]]+(add|set-url)([[:space:]]|$)' <<< "$cmd"; then
    dd_emit_deny no-remote-create "remote attach blocked: 'git remote add' / 'git remote set-url' connects local history to an external destination the operator may not have authorized. Remote wiring is the step that turns a local repo into internet-adjacent state, so it needs explicit operator approval through the host's manual escape/approval path."
  fi
}
