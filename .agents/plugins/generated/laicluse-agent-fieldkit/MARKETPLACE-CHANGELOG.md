...
## [2026-06] whats-new reaches Codex

`laicluse-agent-fieldkit` now ships to Codex with `$whats-new`. The Codex
variant uses `codex plugin list --json` for installed plugin paths and reads the
latest CHANGELOG section directly, so the command works from generated Codex
plugin roots without touching broadcast sentinels.
