...
2. Read `cron_job_id` from the file (canonical marker list in `autonomous:keepalive`, "The return contract"). If it names a live cron job, invoke `autonomous:cron` via the Skill tool to `CronDelete` that id. When the value is `none (persistent process)`, `none (self-check heartbeat)`, `paused`, `stopped`, or `failed`, there is no live cron to cut; skip the CronDelete and proceed. The `none (self-check heartbeat)` case has a wake-up heartbeat rather than a cron; it ends simply by `stop` not scheduling another wake-up (the already-fired one was the last), so there is nothing to cancel here either.
