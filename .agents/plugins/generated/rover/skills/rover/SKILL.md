...
2. Claude asks the autonomy layer (`autonomous:keepalive`) whether it needs to keep itself alive, and how. In an interactive TUI it does, so keepalive starts a `CronCreate` job that re-enters this conversation every minute while the REPL is idle, carrying a prompt that tells Claude to read the loop file and act on the current phase. In a persistent process that can self-pace (a conveyor line, where the runtime exposes a wake-up hook but no cron) keepalive schedules a self-check heartbeat via `autonomous:selfcheck` instead: a periodic wake-up that re-engages a quietly-ended turn and beats the host's stall timer. Only a pure batch run with neither hook gets nothing and drives the phase machine to completion in one run.
...
**Interactive or persistent: the autonomy layer decides, not the caller.** At setup the rover invokes `autonomous:keepalive`, which probes the runtime and schedules whichever heartbeat fits: a cron (interactive session), a self-check wake-up (persistent process that can self-pace, such as a conveyor line), or nothing (a pure batch run that drives straight to completion). The caller never has to tell the rover which mode it is in or instruct it to "skip the cron"; the probe makes that call. Whichever mode, the loop-file discipline is identical, so an interactive session can `wake` a dropped mission later. See `autonomous:keepalive` for the probe and its one load-bearing assumption.
...
1. Invoke `autonomous:keepalive` via the Skill tool. Pass `.autonomous/<NAME>.md` as the loop-file path (derive `<NAME>` from the Dispatch the same way step 5 does: ALL-CAPS, hyphens, no spaces, goal not mechanism; no tool call needed for this, it comes out of reading the Dispatch text). Keepalive returns one of three values for the loop file's `cron_job_id`: a cron job id (interactive session, cron heartbeat scheduled), `none (self-check heartbeat)` (persistent process that can self-pace, wake-up heartbeat scheduled), or `none (persistent process)` (pure batch run, no heartbeat); hold that value in-session to write into the loop file in step 5. How the probe decides is keepalive's business; see `autonomous:keepalive`. The probe goes first so that, in an interactive session, the heartbeat is the safety net during setup's generation-horizon hazards (the big template write in step 5 is the worst), not a consequence of surviving them. The loop file does not have to exist yet; an scheduled cron's Read is a no-op when the file is missing, and the next tick picks up once the file lands. If the prelaunch question in step 2 ends the mission (operator refuses the no-git-repo proposal) and a heartbeat was scheduled, invoke `autonomous:cron` with `CronDelete` on the returned id before stopping so the cron does not outlive the abort.
...
cron_job_id: <set by autonomous:keepalive at setup; one of: a live job id | none (self-check heartbeat) | none (persistent process) | paused | stopped | failed>
...
**Persistent mode has no STANDBY cron.** When `cron_job_id` is `none (persistent process)`, the process drove the phase machine to completion in one pass and there is no heartbeat. When it is `none (self-check heartbeat)`, the process had a wake-up heartbeat that kept it alive through the phases; reaching STANDBY-and-stop ends that heartbeat simply by not scheduling another wake-up (`stop` does this, see its step 2). In both persistent modes STANDBY has no idle-backoff role: run the entry check once and end the mission through `stop`. The cron-management steps below stay as written; `autonomous:cron` no-ops on either sentinel (see its "No cron, no-op" section), so they do nothing without any special-casing here.
