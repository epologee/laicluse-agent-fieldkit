...
The operator just cursed "shit" at the session. This is a capture, not a request
...
1. Distil what actually set them off into a plain one-line pointer of at most a dozen
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/bin/anger-log" shit <<'CAPTURE_NOTE'
   <pointer>
   CAPTURE_NOTE
   ```

   No clear cause? Log the word alone, do not invent one:
   `node "${CLAUDE_PLUGIN_ROOT}/bin/anger-log" shit </dev/null`
